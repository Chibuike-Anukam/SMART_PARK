...
const STATUS_COLORS = {
    ...
    unknown: "rgba(239, 68, 68, 0.45)",
};

function spotStatusColor(status) {
    return status === "occupied" ? STATUS_COLORS.occupied : STATUS_COLORS.free;
}

function isAvailableSpot(status) {
    return status === "free" || status === "accessible";
}
...
function freeSpotNodeIds() {
    return lotData.spots
        .filter((s) => isAvailableSpot(s.status))
        .map((s) => `node_${s.id}`);
}
...
function draw() {
    ...
    for (const spot of lotData.spots) {
        const color = spotStatusColor(spot.status);
        const x0 = spot.x - spot.w / 2;
        const y0 = spot.y - spot.h / 2;
        const nodeId = `node_${spot.id}`;
        const isSelected = nodeId === selectedSpotNodeId;
        ctx.fillStyle = color;
    ...
    for (const node of lotData.nodes) {
        ...
        ctx.fill();
        ctx.strokeStyle = isOccupied ? "#991b1b" : "rgba(15, 23, 42, 0.85)";
        ctx.lineWidth = isSelected ? 2.5 : 1.5;
        ctx.stroke();
    }
